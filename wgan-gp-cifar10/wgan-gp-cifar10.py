...
class Generator(chainer.Chain):
    """wgan-gp for the cifar10 dataset"""
    """(batch_size, n_hidden/z_dim) -> (batch_size, 3, 32, 32) """
    ...
    def __init__(self, n_hidden=100, bottom_width=3, ch=512):
        ...
        with self.init_scope():
            ...
            self.dc1 = L.Deconvolution2D(in_channels=ch, out_channels=ch//2, ksize=3, stride=2, pad=1)
            # output size (batch_size, 256, 5, 5)

            self.dc2 = L.Deconvolution2D(in_channels=ch//2, out_channels=ch//4, ksize=3, stride=2, pad=1)
            # output size (batch_size, 128, 9, 9)

            self.dc3 = L.Deconvolution2D(in_channels=ch//4, out_channels=ch//8, ksize=2, stride=2, pad=1)
            # output size (batch_size, 64, 16, 16)

            self.dc4 = L.Deconvolution2D(in_channels=ch//8, out_channels=3, ksize=4, stride=2, pad=1)
            # output size (batch_size, 3, 32, 32)
...
class Critic(chainer.Chain):
    """wgan-gp for the cifar10 dataset"""
    """(batch_size, 3, 32, 32) -> (batch_size, 1) """

    def __init__(self, ch=512):
        super(Critic, self).__init__()
        with self.init_scope():
            self.c0 = L.Convolution2D(in_channels=3, out_channels=64, ksize=4, stride=2, pad=1)
            # output size (batch_size , 64, 16, 16)

            self.c1 = L.Convolution2D(in_channels=ch // 8, out_channels=128, ksize=2, stride=2, pad=1)
            # output size (batch_size, 128, 9, 9)

            self.c2 = L.Convolution2D(in_channels=ch // 4, out_channels=256, ksize=3, stride=2, pad=1)
            # output size (batch_size, 256, 5, 5)
...
class WGANGPUpdater(training.StandardUpdater):
    ...
    def update_core(self):
        ...
        for _ in range(n_critic):
            ...
            loss_gp = self.lam * F.mean_squared_error(grad, xp.ones_like(grad.data))
...
def main():
    ...
    train, _ = datasets.get_cifar10(withlabel=False, ndim=3, scale=1.)  # ndim=3 : (ch,width,height)
